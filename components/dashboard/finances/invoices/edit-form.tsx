...
export function EditInvoiceForm({ invoice, clients }: { invoice: FullInvoice, clients: Client[] }) {
  const initialState: InvoiceState = { message: null, errors: {} };
  const updateInvoiceWithId = updateInvoice.bind(null, invoice.id);
  const [state, formAction] = useActionState(updateInvoiceWithId, initialState);

  // Estado del formulario
  const [lineItems, setLineItems] = useState<FormLineItem[]>(invoice.line_items.map(item => ({ ...item, total_price: item.quantity * item.unit_price })));
  ...
  return (
    <form action={formAction} className="space-y-6">
        ...
        <CardContent className="space-y-6 pt-10 px-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-4">
            ...
            <div className="p-4 ">
              ...
              {state.errors?.client_id && (
                <p className="text-sm text-red-500 mt-2">{state.errors.client_id[0]}</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 py-6 px-4 bg-neutral-300/20 border border-neutral-200/30 rounded-xl shadow-sm text-xs">
            <div className='w-full space-y-2'>
              ...
              <Input value={`INV-0000${invoice.invoice_number}/25`} className='border border-white rounded-xl w-full px-4 py-4 font-semibold' readOnly disabled />
            </div>
            ...
            <div className='w-full space-y-2 text-xs'>
              ...
              <DatePicker name="issue_date" defaultValue={new Date(invoice.issue_date)} />
            ...
            <div className='w-full space-y-2 text-xs'>
              ...
              <DatePicker name="due_date" defaultValue={new Date(invoice.due_date)} />
                ...
                <TableBody>
                  {lineItems.map((item, index) => (
                    <TableRow key={item.id || index}>
                      ...
                      <TableCell className="text-right text-xs font-semibold">{formatCurrency(item.total_price || 0)}</TableCell>
                      <TableCell>
      ...
      <div className="flex justify-end gap-4 w-full max-w-5xl m-auto pr-3">
        ...
        <Button type="submit" className='transition-all hover:scale-105 text-success shadow-sm border border-success '>Actualizar Factura</Button>
      </div>
    </form>
  );
}
